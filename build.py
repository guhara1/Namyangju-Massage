#!/usr/bin/env python3
"""간다GO 남양주 출장마사지 — 정적 사이트 빌드 스크립트.

content/ 패키지의 페이지 정의를 읽어 정적 HTML을 생성한다.

규칙(자동 적용):
  - 본문 텍스트 2,000자 미만 페이지는 robots noindex 처리
  - sitemap.xml·rss.xml 에는 index 허용 페이지만 포함
  - 모든 페이지에 WebPage·BreadcrumbList 구조화 데이터 자동 삽입
  - robots.txt, IndexNow 키 파일({KEY}.txt), urls.txt 자동 생성

빌드 후 색인 통보:
  python3 build.py && python3 notify_indexnow.py   # 빙·네이버·얀덱스 즉시 통보
"""
import html
import json
import os
...
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;700&family=Noto+Serif+KR:wght@700;900&display=swap" media="print" onload="this.media='all'">
<noscript><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;700&family=Noto+Serif+KR:wght@700;900&display=swap"></noscript>
...
<footer class="site-footer">
  <div class="container footer-grid">
    <div class="footer-col footer-about">
      ...
      <p class="footer-desc">경기도 남양주시 전지역 방문 출장마사지·홈타이 안내 사이트입니다. 모든 서비스는 안내된 관리 범위와 위생·안전 기준 안에서만 제공됩니다.</p>
      <address class="footer-contact">
        ...
        <span class="footer-contact-row"><span class="footer-label">서비스 지역</span> 경기도 남양주시 전지역</span>
      </address>
    </div>
    <nav class="footer-col" aria-label="지역 안내">
      <p class="footer-title">지역 안내</p>
      <ul>
        <li><a href="/namyangju/dasan-dong-chuljangmassage/">다산동 출장마사지</a></li>
        <li><a href="/namyangju/byeollae-dong-chuljangmassage/">별내동 출장마사지</a></li>
        <li><a href="/namyangju/hopyeong-dong-chuljangmassage/">호평동 출장마사지</a></li>
        <li><a href="/namyangju/wabu-eup-chuljangmassage/">와부읍 출장마사지</a></li>
        <li><a href="/namyangju/jinjeop-eup-chuljangmassage/">진접읍 출장마사지</a></li>
      </ul>
    </nav>
    <nav class="footer-col" aria-label="지하철역 안내">
      <p class="footer-title">역세권 안내</p>
      <ul>
        <li><a href="/namyangju/dasan-station-chuljangmassage/">다산역</a></li>
        <li><a href="/namyangju/byeollae-station-chuljangmassage/">별내역</a></li>
        <li><a href="/namyangju/pyeongnae-hopyeong-station-chuljangmassage/">평내호평역</a></li>
        <li><a href="/namyangju/maseok-station-chuljangmassage/">마석역</a></li>
      </ul>
    </nav>
  ...
  <div class="footer-bottom">
    <div class="container footer-bottom-inner">
      ...
      <p class="footer-note">경기도 남양주시를 안내하는 건전한 방문 관리 사이트이며, 불법적인 요청은 어떤 경우에도 응하지 않습니다.</p>
...
def build() -> None:
    report = []
    sitemap_pages = []  # (url, page) — index 허용 페이지만
    site = BASE_URL.rstrip("/")
    now = datetime.now(timezone.utc)
    lastmod = now.strftime("%Y-%m-%d")

    for page in PAGES:
        path = page["path"]  # "" 또는 "namyangju/.../" 형태
        ...
        noindex = page.get("noindex", False) or chars < MIN_INDEX_CHARS
        if not noindex:
            sitemap_pages.append((site + "/" + path, page))
        report.append((path or "/", chars, "noindex" if noindex else "index"))

    # sitemap.xml (lastmod 포함 — 크롤러 재방문 신호)
    urls = "\n".join(
        f"  <url><loc>{u}</loc><lastmod>{lastmod}</lastmod></url>"
        for u, _ in sitemap_pages
    )
    ...
    # rss.xml (RSS 2.0 피드 — 네이버 등 피드 기반 수집 보조)
    pub = format_datetime(now)
    items = []
    for u, page in sitemap_pages:
        items.append(
            "    <item>\n"
    ...
    with open(os.path.join(ROOT, "rss.xml"), "w", encoding="utf-8") as f:
        f.write(
            ...
            "  <channel>\n"
            f"    <title>{html.escape(BRAND)} 남양주 출장마사지·홈타이</title>\n"
            f"    <link>{site}/</link>\n"
            f'    <atom:link href="{site}/rss.xml" rel="self" type="application/rss+xml"/>\n'
            "    <description>남양주시 읍·면·동·역세권별 출장마사지·홈타이 방문 예약 안내</description>\n"
    ...
    # IndexNow 키 파일 — 루트에 {KEY}.txt (내용도 키 그 자체)
    with open(os.path.join(ROOT, f"{INDEXNOW_KEY}.txt"), "w", encoding="utf-8") as f:
        f.write(INDEXNOW_KEY + "\n")

    # 색인 통보용 URL 목록 (notify_indexnow.py 가 사용)
    with open(os.path.join(ROOT, "urls.txt"), "w", encoding="utf-8") as f:
        f.write("\n".join(u for u, _ in sitemap_pages) + "\n")
    ...
    print(f"\n{len(report)} pages built, {len(sitemap_pages)} in sitemap/rss.")
